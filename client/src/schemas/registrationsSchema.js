...
export const registrationsSchema = yup.object().shape({
  carnet: yup
    .string()
    .min(6, 'El carnet debe tener al menos 6 caracteres')
    .matches(carnetRegex, { message: 'Introduce un carnet válido' })
    .required('El carnet es requerido'),
  nombre: yup
    .string()
    .matches(nombreRegex, {
      message: 'El nombre no debe incluir caracteres numericos'
    })
  ...
  telefono: yup
    .string()
    .matches(telefonoRegex, 'Introduce un teléfono de 8 números')
    .required('El telefono es requerido'),
  fecha_nacimiento: yup
    .string()
    .required('La fecha de nacimiento es requerida'),
  carrera: yup.string().required('La carrera del estudiante es requerida'),
  genero_poesia: yup
    .string()
